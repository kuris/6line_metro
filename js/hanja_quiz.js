...
const stationKanji = {
  ...
  "신용산": { hanja: "NewYongsan", meaning: "새 신, 용 룡, 산 산", hint: "새로운 용의 산", sanityBonus: 5, fakes: ["매울 신, 얼굴 용, 산 산", "믿을 신, 쓸 용, 뫼 산"] },
  ...
  "상월곡": { hanja: "상월곡", meaning: "위 상, 달 월, 골짜기 곡", hint: "위쪽 달빛 골짜기", sanityBonus: 5, fakes: ["항상 상, 한 달 월, 굽을 곡", "코끼리 상, 넘을 월, 곡식 곡"] },
...
async function maybeRunHanjaQuiz(st) {
  ...
  TrainPanel.addLog(`[승객 식별 제어] 고대 문자 해독 모듈 가동`, 'warn');
  sfx.modem();
  ...
  options.push({ text: "단념한다 (모르겠다)", correct: false, isGiveUp: true });

  await seq([
    ['', 'blank', 0],
    [`"...다음 역은 ${st.name}, ${st.name}역입니다."`, 'announce', 800],
    [`${st.name}... ${data.hanja}.`, 'narrator', 1800],
    ['어떠한 금기로 이 문자가 구성되었던가.', 'system', 3000],
    ['', 'blank', 3500]
  ]);
      ...
      return [
        ...
        async () => {
          if (opt.correct) {
            ...
            updateStats();
            TrainPanel.addLog('[해독 승인] 죽은 자들의 언어를 읽었습니다', 'life');
            sfx.item();
            await seq([
              [`맞아, ${data.hanja} (${data.meaning}).`, 'highlight', 1000],
              [`${data.hint}라는 의미지. 낯설던 표지판이 다시 선명해진다.`, 'narrator', 2200],
              ['魂(혼) +5 — 잠시 동안의 평온이 찾아온다.', 'life', 3500],
              ['', 'blank', 4000]
            ]);
            resolve(true);
            ...
            updateStats();
            TrainPanel.addLog('[해독 단념] 이해할 수 없는 기호에 눈이 멀었습니다', 'warn');
            sfx.boom(0.1);
            await seq([
              ['단념했다. 애초에 산 자의 언어가 아닐지 모른다.', 'narrator', 1200],
              ['표지판의 글자들이 벌레처럼 꿈물거리며 시야를 어지럽힌다.', 'death', 2500],
              ['魂(혼) -3 — 싹트는 불신.', 'death', 3800],
              ['', 'blank', 4500]
            ]);
            resolve(true);
          } else {
            G.hanjaFail++;
            G.sanity = Math.max(0, G.sanity - 5);
            updateStats();
            TrainPanel.addLog('[치명적 오류] 오독으로 인한 인지 체계 붕괴', 'warn');
            if (window.HorrorFX) window.HorrorFX.flashRed(600);
            sfx.boom(0.1);
            await seq([
              [`${opt.text}... 였던가?`, 'narrator', 1000],
              ['아니, 문자가 뒤틀려 보인다! 표지판이 날카로운 비명으로 부서진다.', 'death', 2200],
              ['잘못된 해독은 생존 본능을 뒤흔들고 뇌리에 낙인을 찍는다.', 'death', 3500],
              ['魂(혼) -5 — 타들어 가는 정신.', 'death', 4500],
              ['', 'blank', 5000]
            ]);
            resolve(true);
          }
